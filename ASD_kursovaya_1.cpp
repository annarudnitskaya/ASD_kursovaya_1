...
string change_functions(string str) {
	...
	int val4 = str.find("abs");
	if (val4 != -1) {
		str.replace(str.find("abs"), 3, "a");
	}
...
int main() {
	...
	try {
		if (/*check_bracekts(str) &&*/ check_for_correct_input(str) /*&& chek_kol_(str)*/) {
			postfix_string = postfix(str); // convert to postfix
		}
	}
